...
function downloadCsv(report: AgingReport, filename: string) {
  const headers = ['Vendor', 'Current', '1-30 Days', '31-60 Days', '61-90 Days', '91+ Days', 'Total'];
  ...
  const lines = [
    `"A/P Aging Report — As Of ${asOfDate}"`,
    '',
...
export default function ApAgingPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api.get<AgingReport>('/api/reports/ap-aging');
      setReport(data);
    } catch (err) {
        ...
        action={
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => {
                if (report) downloadCsv(report, 'ap-aging.csv');
              }}
            >
              Download CSV
            </Button>
